...
(function () {
  ...
  const defaultData = () => {
    ...
    return {
      ...
      properties: [
        { id: p1, code: "P-2001", name: "グランドレジデンス品川", type: "マンション", address: "東京都港区港南2-1-1", price: 78000000, layout: "3LDK", area: 72.5, status: "active", agentId: a1 },
        { id: p2, code: "P-2002", name: "世田谷 新築戸建", type: "戸建", address: "東京都世田谷区成城6-3-2", price: 96800000, layout: "4LDK", area: 105.2, status: "negotiating", agentId: a2 },
        { id: p3, code: "P-2003", name: "横浜みなとみらい 事業用地", type: "土地", address: "神奈川県横浜市西区みなとみらい4", price: 250000000, layout: "—", area: 480, status: "prep", agentId: a1 },
      ],
      customers: [
      ...
      deals: [
        { id: uid(), title: "山田様 品川MS", customerId: c1, propertyId: p1, amount: 76000000, stage: "viewing", nextDate: "" },
        { id: uid(), title: "佐藤様 世田谷戸建", customerId: c2, propertyId: p2, amount: 95000000, stage: "offer", nextDate: "" },
      ],
      viewings: [],
    };
  };
  ...
  function renderProperties() {
    ...
    const rows = data.properties
      ...
      .map((p) => {
        ...
        return `<tr>
          <td>${esc(p.code)}</td><td>${esc(p.name)}</td><td>${esc(p.type)}</td>
          <td>${esc(p.address)}</td>
  ...
  function renderDashboard() {
    ...
    const closedThisMonth = data.deals.filter((d) => d.stage === "closed" && (d.nextDate || "").slice(0, 7) === thisMonth);
    ...
    $("#activeProps").innerHTML = activeProps.length
      ? activeProps.slice(0, 6).map((p) => { const st = propStatusMeta(p.status);
          return `<div class="mini-item"><span class="mi-main">${esc(p.name)}<br><span class="mi-sub">${esc(p.type)} / ${esc(p.layout)} / ${esc(p.address)}</span></span>
